...
outputs:
  - id: matches
    ...
    type: File
    outputBinding:
      glob: $(inputs.covariance_model_database.nameroot).cmsearch_matches.tbl
  - id: programOutput
    label: 'direct output to file, not stdout'
    type: File
    outputBinding:
      glob: $(inputs.covariance_model_database.nameroot).cmsearch.out
doc: >
...
arguments:
  - position: 0
    prefix: '--tblout'
    valueFrom: $(inputs.covariance_model_database.nameroot).cmsearch_matches.tbl
  - position: 0
    prefix: '-o'
    valueFrom: $(inputs.covariance_model_database.nameroot).cmsearch.out
requirements:
